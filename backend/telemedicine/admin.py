...
class appointmentsAdmin(admin.ModelAdmin):
    list_display = ('appointmentUID', 'paitientProfileUID', 'doctorProfileUID', 'date', 'time', 'locationUID', 'doctors_notes', 'reports')

class inboxAdmin(admin.ModelAdmin):
    list_display = ('inboxUID', 'userGUID', 'toDemoUID', 'subject', 'body', 'date', 'time', 'documents', 'isRead')

class reportsAdmin(admin.ModelAdmin):
    list_display = ('reportUID', 'appointmentUID', 'date', 'time', 'details', 'documents')

class insuranceAdmin(admin.ModelAdmin):
    list_display = ('insuranceUID', 'name', 'accountid', 'insuranceType')

class locationAdmin(admin.ModelAdmin):
    list_display = ('locationUID', 'name', 'address1', 'address2', 'city', 'State', 'zip')

class notificationAdmin(admin.ModelAdmin):
    list_display = ('notificationUID', 'appointmentUID', 'reportUID', 'inboxUID', 'messages', 'link', 'date', 'time', 'doEmail')
